...
# ============================================================================
# SPEC 6: Funções de Comunicação stdin/stdout (JSON-por-linha)
# ============================================================================

def read_command_from_stdin():
    """Lê o payload de comando completo do stdin."""
    try:
        input_data = sys.stdin.read()
        if not input_data:
            raise ValueError("FINIT0 - No data received from stdin.")
        return json.loads(input_data)
    except Exception as e:
        # Se falharmos aqui, precisamos logar no stderr, pois o stdout é para JSON
        print(f"FBIN0 - Failed to parse command from stdin: {e}\n{traceback.format_exc()}", file=sys.stderr)
        sys.exit(1)

def send_message(msg_type, payload):
    """Envia uma mensagem JSON-por-linha para o stdout."""
    message = json.dumps({"type": msg_type, "payload": payload})
    print(message, flush=True)  # flush=True é essencial


# ============================================================================
# Caminhos legados removidos - agora todos os caminhos vêm do main.js
# ============================================================================

# As seguintes constantes foram REMOVIDAS:
# - get_resource_path()
# - BASE_PATH
# - UPLOAD_FOLDER_BUILD / UPLOAD_FOLDER_DIST
# - RECIPE_PATH_BUILD / RECIPE_PATH_DIST
# - MODELS_PATH_BUILD / MODELS_PATH_DIST
# - CONFIG_PATH_BUILD / CONFIG_PATH_DIST
# - CUSTOM_PRELOADING_PATH_BUILD / CUSTOM_PRELOADING_PATH_DIST
...
class Recipe:
    """
    A classe Recipe foi REFATORADA para não mais depender de arquivos de receita.
    Agora recebe os comandos diretamente do payload stdin.
    Esta classe é mantida para compatibilidade, mas será simplificada.
    """
    def __init__(self, commands_from_payload=None):
        """
        Inicializa a Recipe a partir do payload recebido via stdin.
        
        Args:
            commands_from_payload (dict): Comandos extraídos do payload JSON.
        """
        self.EXPORT_FORMATS = {'csv', 'json', 'pdf', 'connected_database'}
        self.COMMANDS_SPEC = {'white_background', 'export_separation'}
        self.commands_data = commands_from_payload or {}
        
        if DEBUG:
            print(f"DEBUG - Recipe initialized with commands: {self.commands_data}")

    def process_commands(self) -> Dict[str, Any]:
        """
        Processa os comandos recebidos do payload.

        Returns:
            dict: Contém commands, commands_spec e exportation_format.
        """
        if DEBUG:
            print(f"DEBUG - Processing commands: {self.commands_data}")
    ...
    def _decompose_commands(self, commands: Dict[str, bool]) -> Tuple[Dict[str, bool], Dict[str, bool], Dict[str, bool]]:
        """
        Decompõe comandos em três categorias:
        - Commands para comandos gerais,
        - Commands_spec para configurações especiais,
        - Exportation_format para comandos específicos de exportação.

        Args:
            commands (dict): Os comandos brutos do payload.

        Returns:
            tuple: Contém dicionários de commands, commands_spec e exportation_format.
        """
        general_commands = {}
        command_specs = {}
        export_formats = {}
...
class Factory:
    ...
    def __init__(self):
        """
        Inicializa o Factory object a partir do comando recebido via stdin.
        
        SPEC 7: Refatoração completa - não mais adivinha caminhos.
        Todos os caminhos vêm do payload do main.js.
        
        Raises:
            ValueError: Se caminhos essenciais estiverem faltando.
        """
        try:
            # 1. Ler o comando do stdin
            send_message("log", {"level": "info", "message": "Factory: Reading command from stdin..."})
            command_payload = read_command_from_stdin()
            
            # 2. Configurar o Factory a partir do payload
            self.commands_raw = command_payload.get('commands', {})
            self.config = command_payload.get('config', {})
            
            # 3. Obter caminhos ABSOLUTOS do main.js
            paths = command_payload.get('paths', {})
            self.models_path = paths.get('models_dir')
            self.upload_folder = paths.get('uploads_dir')
            self.output_folder = paths.get('results_dir')
            
            # 4. Obter lista de arquivos (NÃO MAIS O `load_images`)
            self.files_to_process = command_payload.get('files_to_process', [])
            
            # Validar se os caminhos essenciais foram recebidos
            if not all([self.models_path, self.upload_folder, self.output_folder]):
                raise ValueError("FINIT1 - Missing essential paths from main process.")
            
            # Processar comandos usando a classe Recipe refatorada
            recipe = Recipe(self.commands_raw)
            recipe_content = recipe.process_commands()
            ...
            # Configurar imagens a partir dos nomes de arquivo recebidos
            self.images = {}
            ...
            send_message("log", {
                "level": "info", 
                "message": f"Factory initialized. Ready to process {len(self.files_to_process)} images."
            })
            
        except Exception as e:
            send_message("error", {
                "code": "FINIT_FATAL",
                "message": f"Factory initialization failed: {e}",
    ...
    def load_config(self):
        """
        Retorna a configuração que foi passada via stdin.
        
        REFATORADO: Não mais lê de arquivo, usa a config do payload.
        
        Returns:
            dict: O conteúdo da configuração.
        """
        return self.config
    ...
    def load_custom_preloading(self):
        """
        REFATORADO: Custom preloading agora pode vir no payload de config.
        
        Returns:
            dict: Configurações de preloading customizado, se houver.
        """
        try:
            # Tentar obter custom preloading da config
            return self.config.get('custom_preloading', {})
        except Exception as e:
            send_message("log", {"level": "warning", "message": f"LCP1 - Error while loading custom preloading: {e}"})
            return {}
    ...
    def get_output_folder(self):
        ...
        REFATORADO: Usa o caminho passado via stdin.
        
        Returns:
            str: O caminho para o diretório de saída.
        """
    ...
    def verify_preprocessed_images(self):
        ...
        """
        if not self.images:
            send_message("error", {"code": "FVPI1", "message": "No images found."})
    ...
    def _execute_command(self, command):
        """
        Executa um único comando e retorna o resultado, com compatibilidade para execução paralela.
        
        SPEC 8: Refatorado para usar send_message para reportar progresso e erros.
        ...
        Returns:
            list: Uma lista de tuplas. Cada tupla contém:
                - image_filename (str): O nome do arquivo da imagem processada.
                - command (str): O comando que foi executado.
                - result_values (various): Os valores de resultado da execução do comando.
                Se ocorrer um erro, a lista conterá uma única tupla com uma mensagem de erro.
        """
        if DEBUG:
        ...
        try:
            ...
            if not binary_path:
                error_msg = f"Model {command} not found."
                if DEBUG:
            ...
            with Utils().change_directory(binary_dir):
                if config['FORCE_MAXPERFORMANCE']:
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future_to_batch = {
                            executor.submit(self._run_subprocess, binary_path, image_paths[i:i + self.batch_size]): i 
                        ...
                        for future in concurrent.futures.as_completed(future_to_batch):
                            ...
                            try:
                                ...
                                batch_results = self._process_results(result_lines, command)
                                ...
                                error_msg = f"Error executing {command} on batch {batch_index}: {e}"
                                if DEBUG:
                                    print(f"DEBUG - {error_msg}, retrying with smaller batch")
                                ...
                                for j in range(batch_index, batch_index + self.batch_size, self.batch_size // 2):
                                    ...
                                    if DEBUG:
                                        print(f"DEBUG - Retrying with smaller batch {j // (self.batch_size // 2) + 1}")
                                    try:
                                        ...
                                        send_message("error", {
                                            "code": "FBIN2_P",
                                            "task": command,
                                            "message": f"Error while retrying smaller batch: {retry_e}",
                                            "trace": traceback.format_exc()
                                        })
                                        traceback.print_exc()
                    ...
                    for i in range(0, len(image_paths), self.batch_size):
                        ...
                        try:
                            ...
                            error_msg = f"Error while executing {command} on batch"
                            if DEBUG:
                                print(f"DEBUG - {error_msg}, retrying with smaller batch: {e}")
                            
                            send_message("log", {
                                "level": "warning",
                                "message": f"{error_msg}: {e}"
                            })
            ...
            send_message("error", {
                "code": "FEXEC",
                "task": command,
                "message": f"Command execution failed: {outer_e}",
                "trace": traceback.format_exc()
            })
    ...
    def execute_recipes_parallel(self):
        """
        Executa as receitas carregadas em paralelo usando multiprocessing.Pool.
        
        SPEC 8: Refatorado para usar send_message para reportar status.
        ...
        send_message("log", {
            "level": "info",
            "message": f"Starting parallel execution of {len(self.true_commands)} commands..."
        })
        ...
        try:
            ...
            error_msg = f"Error while executing recipes in parallel: {e}"
            send_message("error", {
                "code": "FPAR3",
                "message": error_msg,
                "trace": traceback.format_exc()
            })
            print(f"FPAR3 - {error_msg}")
            traceback.print_exc()
            return
        ...
        for result in results:
            try:
                if isinstance(result, list):
                    ...
                    send_message("log", {
                        "level": "warning",
                        "message": f"Expected a list result, got {type(result)} instead."
                    })
            except Exception as e:
                error_msg = f"Error while processing parallel execution results: {e}"
                send_message("error", {
                    "code": "FPAR4",
                    "message": error_msg,
                    "trace": traceback.format_exc()
                })
                print(f"FPAR4 - {error_msg}")
                traceback.print_exc()
                return
...
def handle_parallel_recipes_execution(__name__, Factory):
    """
    Executa receitas em paralelo ou sequencialmente com base no nome do módulo atual.
    
    SPEC 8: Refatorado para usar send_message para todos os logs e erros.

    Args:
        __name__ (str): O nome do módulo atual.
        Factory (class): A classe Factory usada para executar receitas.
    ...
    if __name__ == "__main__":
        try:
            ...
            send_message("error", {
                "code": "FPAR_MASTER",
                "message": f"Error in parallel execution, switching to sequential execution: {e}",
                "trace": traceback.format_exc()
            })
            print(f"FPAR_MASTER - Error in parallel execution: {e}", file=sys.stderr)
            traceback.print_exc()
            ...
            try:
                ...
                send_message("log", {
                    "level": "info",
                    "message": "Attempting sequential execution as fallback..."
                })
                factory.execute_recipe()
            except Exception as e_seq:
                send_message("error", {
                    "code": "FPAR_MASTER_SEQ",
                    "message": f"Error during sequential execution fallback: {e_seq}",
                    "trace": traceback.format_exc()
                })
                print(f"FPAR_MASTER - Error during sequential execution fallback: {e_seq}", file=sys.stderr)
                traceback.print_exc()
                sys.exit(1)
            ...
            print("FPAR_MASTER - Execution interrupted by user.", file=sys.stderr)
            sys.exit(130)
